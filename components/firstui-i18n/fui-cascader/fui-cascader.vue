<template>
	<!--本文件由FirstUI授权予马*健（会员ID：3   156，身份证尾号：01   04 3X）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
		...
		methods: {
			...
			subLevelData(data, layer) {
				if (!data || data.length === 0) {
					...
					let item = [{
						[this.textKey]: this.text || this.textI8n,
						[this.valueKey]: '',
						[this.srcKey]: '',
						index: -1,
				...
				if (vals.length > 0) {
					...
					if (this.stepLoading) {
						options.forEach((item, index) => {
							...
							let obj = item[subi] || {}
							selectedArr.push({
								[this.textKey]: obj[this.textKey] || this.text || this.textI8n,
