<template>
	<!--本文件由FirstUI授权予马*健（会员ID： 3 1 56，身份证尾号： 01 0  43X）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
	<view @tap="maskClose" :style="{background:maskBackground}" class="fui-dialog__wrap"
		...
		<view class="fui-dialog__inner" :style="{background:background,borderRadius:radius+'rpx'}" @tap.stop="stop">
			<text class="fui-dialog__title" :style="{color:color}" v-if="title!==true">{{title || titleTextI18n}}</text>
			...
			<view class="fui-dialog__footer">
				<text v-for="(item,index) in btns" :key="index" :style="{color:item.color || '#333333'}"
...
<script>
	...
	export default {
		...
		computed: {
			confirmTextI18n() {
				return t("fui-dialog.confirm")
			},
			cancelTextI18n() {
				return t("fui-dialog.cancel")
			},
			titleTextI18n() {
				return t("fui-dialog.title")
			},
			localeTextI18n() {
				return t("fui-dialog.locale")
			}
		},
		methods: {
			getDefBtns() {
				const btns = [{
					text: this.cancelTextI18n
				}, {
					text: this.confirmTextI18n,
					color: primaryColor
				}];
				this.getButtons(this.buttons.length == 0 ? btns : this.buttons)
			},
			getButtons(vals) {
				this.btns = [];
				this.$nextTick(() => {
