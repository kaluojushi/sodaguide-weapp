<template>
	<!--本文件由FirstUI授权予马*健（会员ID：315   6，身份证尾号： 01   043X）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
	<view class="fui-slideverify__wrap" :style="{width:totalWidth+'px',height:height+'rpx',background:background}">
		<text class="fui-sv__text-tip" :style="{fontSize:size+'rpx',background:getBackground,color:color}"
			:class="{'fui-sv__opacity':isPass}">{{tipTextI18n}}</text>
		...
		<!-- #endif -->
			...
			<!-- #endif -->
				...
				<text :class="{'fui-sv__opacity':!isPass}" class="fui-sv__text-res"
					:style="{fontSize:size+'rpx',color:activeColor,lineHeight:height+'rpx',width:passWidth+'px'}">{{successTextI18n}}</text>
				<!-- #endif -->
				<!-- #ifndef APP-NVUE -->
				<text :class="{'fui-sv__opacity':!isPass}" class="fui-sv__text-res"
					:style="{fontSize:size+'rpx',color:activeColor}">{{successTextI18n}}</text>
				<!-- #endif -->
				<text :class="{'fui-sv__opacity':isPass}" class="fui-sv__text-cover"
					:style="{width:totalWidth+'px',fontSize:size+'rpx',height:height+'rpx',lineHeight:height+'rpx',color:activeColor}">{{tipTextI18n}}</text>
			</view>
...
<script>
	...
	export default {
		...
		computed: {
			...
			tipTextI18n() {
				return t("fui-slide-verify.tip")
			},
			successTextI18n() {
				return t("fui-slide-verify.success")
			}
		},
		watch: {
			width(val) {
