<template>
	<!--本文件由FirstUI授权予马*健（会员ID：3 15  6，身份证尾号： 01  04 3X）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
...
<script>
	...
	export default {
		...
		computed: {
			...
			urlTextI18n() {
				return t("fui-upload-video.url")
			},
			deltipTextI18n() {
				return t("fui-upload-video.deltip")
			},
			deleteTextI18n() {
				return t("fui-upload-video.delete")
			},
			confirmTextI18n() {
				return t("fui-upload-video.confirm")
			},
			loadingTextI18n() {
				return t("fui-upload-video.loading")
			},
			waitTextI18n() {
				return t("fui-upload-video.wait")
			},
			preuploadTextI18n() {
				return t("fui-upload-video.preupload")
			},
			uploadedTextI18n() {
				return t("fui-upload-video.uploaded")
			},
			extensionTextI18n() {
				return t("fui-upload-video.extension")
			},
			sizeTextI18n() {
				return t("fui-upload-video.size")
			}
		},
		methods: {
			...
			getStatusText(progress) {
				let text = this.loadingTextI18n
				if (progress === -1) {
					text = this.waitTextI18n
				} else if (progress === -99) {
					text = this.preuploadTextI18n
				} else if (progress === 100) {
					text = this.uploadedTextI18n
				}
				return text;
			},
			initData(urls) {
					...
					success: (e) => {
						...
						if (this.extension.length > 0) {
							...
							if (format && this.extension.indexOf(format) == -1) {
								const text = this.extensionTextI18n.relpace('_suffix_', this.extension.join(
									','))
								this.toast(text);
								return;
							}
