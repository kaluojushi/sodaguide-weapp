<template>
	...
	<view class="fui-rotate__verify-wrap" :class="{'fui-rv__wrap-show':isShow}"
		...
		<view class="fui-rotate__verify" :style="{width:width+'rpx',background:background}" @tap.stop="stop">
			...
			<text class="fui-rv__title"
				:style="{ color: color, fontSize: size + 'rpx' }">{{ title || titleTextI18n }}</text>
			<text class="fui-rv__descr"
				:style="{ color: descrColor, fontSize: descrSize + 'rpx' }">{{tipTextI18n}}</text>
...
<script>
	...
	export default {
		...
		computed: {
			...
			titleTextI18n() {
				return t("fui-rotate-verify.title")
			},
			tipTextI18n() {
				return t("fui-rotate-verify.tip")
			}
		},
		data() {
			let isNvue = false
