<template>
	<!--本文件由FirstUI授权予马*健（会员ID：   3156，身份证尾号： 0 1 04 3X）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
	<view class="fui-copy__text-wrap">
		...
		<view v-if="showTooltip" class="fui-copy__tooltip-list"
			...
			<text :selectable="false" v-if="!removeCopy || buttons.length === 0" class="fui-copy__tooltip-cell"
				...
				@tap.stop="copyVal">{{copyTextI18n}}</text>
...
<script>
	...
	export default {
		...
		computed: {
			...
			copyTextI18n() {
				return t("fui-copy-text.copy")
			},
			successTextI18n() {
				return t("fui-copy-text.success")
			}
		},
		created() {
			this.transform = {
