<template>
	<!--本文件由FirstUI授权予马*健（会员ID：  3 156，身份证尾号：01  0  43X）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
	<view class="fui-slider__captcha-wrap" :class="{'fui-sc__wrap-show':isShow}"
		...
		<view class="fui-slider__captcha" :style="{background:background}" @tap.stop="stop">
			<text class="fui-sc__title"
				:style="{ width: width + 'px',color: color, fontSize: size + 'rpx' }">{{ title || titleTextI18n }}</text>
			<text class="fui-sc__descr"
				:style="{width: width + 'px',color:descrColor, fontSize: descrSize + 'rpx'}">{{tipTextI18n}}</text>
...
<script>
	...
	export default {
		...
		computed: {
			...
			titleTextI18n() {
				return t("fui-puzzle-verify.title")
			},
			tipTextI18n() {
				return t("fui-puzzle-verify.tip")
			}
		},
		data() {
