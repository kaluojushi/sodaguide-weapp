<template>
	<!--本文件由FirstUI授权予马*健（会员ID： 31  56，身份证尾号：01    043X）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
...
<script>
	...
	export default {
		...
		computed: {
			...
			textI18n() {
				return t("fui-countdown-verify.text")
			},
			sendingTextI18n() {
				return t("fui-countdown-verify.sending")
			},
			sentTextI18n() {
				return t("fui-countdown-verify.sent")
			}
		},
		...
		created() {
			if (this.start) {
				...
				this.showText = this.text || this.textI18n;
				this.clearTimer();
			}
		},
		// #ifndef VUE3
		...
		methods: {
			sendCode(e) {
				...
				this.clearTimer();
				this.status = 2;
				this.showText = this.sending || this.sendingTextI18n;
				this.$emit('send', {
					param: this.param
				});
			},
			doLoop() {
				...
				this.showText = seconds + (this.sent || this.sentTextI18n) ;
