<template>
	<!--本文件由FirstUI授权予马*健（会员ID：  315 6，身份证尾号：0 1 04  3X）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
		...
		<view :key="theme" class="fui-date__picker-content"
			...
			<view class="fui-date__picker-header"
				...
				<text class="fui-dpk_btn-cancel"
					...
					@tap.stop="btnCancel">{{cancelText || cancelTextI18n}}</text>
				...
				<text class="fui-dpk__btn-sure" :class="{'fui-dpk__sure-color':!confirmColor}" :style="confrimStyl"
					@tap.stop="btnConfirm">{{confirmText || confirmTextI18n}}</text>
			</view>
			<view class="fui-dpk__range-wrap" v-if="range">
				<view class="fui-dpk__range-inner"
					...
					<text class="fui-dpk__range-text" :class="{'fui-dpk__range-active':isActive==1}" :style="startStyl"
						@tap.stop="rangeChange($event,1)">{{startDate.result || start || startTextI18n}}</text>
					<text class="fui-dpk__range-text" :class="{'fui-dpk__range-active':isActive==2}" :style="endStyl"
						@tap.stop="rangeChange($event,2)">{{endDate.result || end || endTextI18n}}</text>
				</view>
			</view>
				...
				<picker-view :mask-top-style="theme==='dark'?darkStyle:''"
					...
					<picker-view-column v-if="dateType==1 || dateType==2 || dateType==3 || dateType==4 || dateType==5">
						<text :style="contentStyl" v-for="(item,index) in years" :key="index"
							...
							:class="{'fui-dpk__color-dark':theme==='dark'}">{{formatVal(item,'年',unit)}}</text>
					</picker-view-column>
					<picker-view-column v-if="dateType==2 || dateType==3 || dateType==4 || dateType==5">
						<text :style="contentStyl" v-for="(item,index) in months" :key="index"
							class="fui-date__picker-text"
							:class="{'fui-dpk__color-dark':theme==='dark'}">{{formatMonthVal(item,unit)}}</text>
					</picker-view-column>
					<picker-view-column v-if="dateType==3 || dateType==4 || dateType==5">
						<text :style="contentStyl" v-for="(item,index) in days" :key="index"
							class="fui-date__picker-text"
							:class="{'fui-dpk__color-dark':theme==='dark'}">{{formatVal(item,'日',unit)}}</text>
					</picker-view-column>
					<picker-view-column v-if="dateType==4 || dateType==5  || dateType==6  || dateType==7">
						<text :style="contentStyl" v-for="(item,index) in hours" :key="index"
							class="fui-date__picker-text"
							:class="{'fui-dpk__color-dark':theme==='dark'}">{{formatVal(item,'时',unit)}}</text>
					</picker-view-column>
					<picker-view-column v-if="dateType==5  || dateType==6  || dateType==7 || dateType==8">
						<text :style="contentStyl" v-for="(item,index) in minutes" :key="index"
							class="fui-date__picker-text"
							:class="{'fui-dpk__color-dark':theme==='dark'}">{{formatVal(item,'分',unit)}}</text>
					</picker-view-column>
					<picker-view-column v-if="dateType==7 || dateType==8">
						<text :style="contentStyl" v-for="(item,index) in seconds" :key="index"
							class="fui-date__picker-text"
							:class="{'fui-dpk__color-dark':theme==='dark'}">{{formatVal(item,'秒',unit)}}</text>
					</picker-view-column>
				</picker-view>
				<!-- #endif -->
			</view>
...
<script>
	...
	export default {
		...
		computed: {
			...
			startTextI18n() {
				return t("fui-date-picker.start")
			},
			endTextI18n() {
				return t("fui-date-picker.end")
			},
			confirmTextI18n() {
				return t("fui-date-picker.confirm")
			},
			cancelTextI18n() {
				return t("fui-date-picker.cancel")
			},
			errorTextI18n() {
				return t("fui-date-picker.error")
			},
			selectTextI18n() {
				return t("fui-date-picker.select")
			},
			rangeTextI18n() {
				return t("fui-date-picker.range")
			}
		},
		watch: {
			initChange(val) {
		...
		methods: {
			...
			formatVal(num, unit, isUnit) {
				const res = num < 10 ? `0${num}` : num + '';
				if (unit) {
					const locale = uni.getLocale()
					if (~locale.indexOf('zh')) {
						return `${res}${isUnit?unit:''}`
					}
				}
				return res;
			},
			formatMonthVal(num, isUnit) {
				const locale = uni.getLocale()
				if (~locale.indexOf('zh')) {
					const res = num < 10 ? `0${num}` : num + '';
					return `${res}${isUnit?'月':''}`
			...
			getStrCount(str) {
				...
				let result = String(str).match(regex);
			...
			getValueToDate(val) {
				...
				try {
					...
					console.log(this.errorTextI18n)
				}
				return values;
			},
				...
				if (this.range) {
					...
					if (!start) {
						if (this.isAuto) {
							...
							if (!end) {
								this.isActive = 2
								this.toast(this.selectTextI18n.replace('_date_', this.end || this.endTextI18n
									.toLocaleLowerCase()))
								return;
							} else {
							...
							this.toast(this.selectTextI18n.replace('_date_', this.start || this.startTextI18n
								.toLocaleLowerCase()))
							return;
						}
					} else if (start && !end) {
