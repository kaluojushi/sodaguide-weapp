...
<script>
	...
	export default {
		...
		computed: {
			errorTextI18n() {
				return t("fui-poster-weex.error")
			},
			sokTextI18n() {
				return t("fui-poster-weex.sok")
			},
			serrorTextI18n() {
				return t("fui-poster-weex.serror")
			},
			messageTextI18n() {
				return t("fui-poster-weex.message")
			},
			contentTextI18n() {
				return t("fui-poster-weex.content")
			},
			confirmTextI18n() {
				return t("fui-poster-weex.confirm")
			}
		},
		created() {
			this.context = null;
		...
		methods: {
			...
			_modal: function(callback, confirmColor, confirmText) {
				uni.showModal({
					title: this.messageTextI18n,
					content: this.contentTextI18n,
					showCancel: true,
					cancelColor: '#B2B2B2',
					confirmColor: confirmColor || "#181818",
					confirmText: confirmText || this.confirmTextI18n,
					success(res) {
						if (res.confirm) {
				...
				this._judgePermissionPhotoLibrary((res) => {
					...
					if (res) {
						uni.saveImageToPhotosAlbum({
							...
							success(res) {
								uni.showToast({
									title: this.sokTextI18n,
									icon: 'none'
								})
							},
							fail(res) {
								uni.showToast({
									title: this.serrorTextI18n,
									icon: 'none'
								})
							}
						})
