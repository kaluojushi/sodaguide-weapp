<template>
	<!--本文件由FirstUI授权予马*健（会员ID：31 5  6，身份证尾号： 0  1 043X）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
...
<script>
	...
	export default {
		...
		computed: {
			...
			minTextI18n() {
				return t("fui-data-tag.min")
			},
			maxTextI18n() {
				return t("fui-data-tag.max")
			}
		},
		data() {
			return {
		...
		methods: {
			...
			checkboxChange(index, model) {
				...
				if (vals.length <= min && i !== -1) {
					const mintip = this.minTextI18n.replace('_min_', min)
					this.toast(mintip)
					return
				}
				const isAllIndex = this.dataList.findIndex(item => item[this.isAllKey])
				let item = this.dataList[index]
				const maxtip = this.maxTextI18n.replace('_max_', max)
				if (isAllIndex !== -1) {
