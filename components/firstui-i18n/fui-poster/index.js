// 本文件由FirstUI授权予马*健（会员ID：3   156，身份证尾号： 010   43X）专用，请尊重知识产权，勿私下传播，违者追究法律责任。
/*!
 * 生成海报
...
const poster = {
	...
	_modal: function(callback, confirmColor, confirmText) {
		...
		const isZh = locale.indexOf('zh') != -1 ? true : false;
		const msg = isZh ? '您还没有开启相册权限，是否立即设置？' :
			"You haven't enabled the photo album permission yet. Do you want to set it up now?";
		const okText = isZh ? "确定" : "OK";
		uni.showModal({
			title: isZh ? '提示' : 'Message',
			content: msg,
			showCancel: true,
			cancelColor: '#B2B2B2',
			confirmColor: confirmColor || "#181818",
			confirmText: confirmText || okText,
			success(res) {
				if (res.confirm) {
		...
		poster._judgePermissionPhotoLibrary((res) => {
			...
			if (res) {
				uni.saveImageToPhotosAlbum({
					...
					success(res) {
						uni.showToast({
							title: isZh ? '图片保存成功' : 'Image saved successfully',
							icon: 'none'
						})
					},
					fail(res) {
						uni.showToast({
							title: isZh ? '图片保存失败' : 'Image saving failed',
							icon: 'none'
						})
					}
				})
