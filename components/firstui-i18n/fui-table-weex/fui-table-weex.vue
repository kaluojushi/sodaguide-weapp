<template>
	...
	<scroll-view fixFreezing="true" class="fui-table__nvue-wrap" :class="{'fui-swiper-page':fixFreezing}"
		...
		<list :offset-accuracy="5" isSwiperList="true" ref="list" fixFreezing="true" :show-scrollbar="false"
			:bounce="false" :style="{height:height>0 || height!=0?height+unit:'auto',width:totalW+'px'}">
					...
					<view class="fui-table--td"
						...
						<template v-if="model.type!==3">
							...
							<text class="fui-table--td-text"
								...
								v-else
								:style="{color:getColColor(model,item[model.prop],index),fontSize:(model.textSize || textSize) +'rpx',width:(model.width+divideW)+'px'}">{{item[model.prop]==0?item[model.prop]:(item[model.prop] || '')}}</text>
						</template>
						<template v-else>
