<template>
	<!--本文件由FirstUI授权予马*健（会员ID：  3 156，身份证尾号： 0   1043X）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
	<view @touchend="stop">
		...
		<view class="fui-share__sheet-wrap"
			...
			<slot>
				<text class="fui-share__sheet-title"
					:style="{fontSize:size+'rpx',color:color}">{{title==='true' || title===true?'':(title || titleTextI18n)}}</text>
			</slot>
			...
			<view class="fui-share__sheet-button" :class="{'fui-ss__safe-weex':iphoneX && safeArea }"
				...
				<text class="fui-share__sheet-btn" :class="{'fui-ss__safe-weex':iphoneX && safeArea}"
					:style="{fontSize:btnSize+'rpx',color:btnColor}" @tap.stop="cancel">{{btnText || cancelTextI18n}}</text>
...
<script>
	...
	export default {
		...
		computed: {
			...
			titleTextI18n() {
				return t("fui-share-sheet.title")
			},
			cancelTextI18n() {
				return t("fui-share-sheet.cancel")
			}
		},
		watch: {
			// #ifdef APP-NVUE
