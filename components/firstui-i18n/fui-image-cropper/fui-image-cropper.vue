<template>
	<!--本文件由FirstUI授权予马*健（会员ID：  315 6，身份证尾号：  0 104 3X）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
...
<script>
	...
	export default {
		...
		computed: {
			loadingTextI18n() {
				return t("fui-image-cropper.loading")
			},
			netImgErrorI18n() {
				return t("fui-image-cropper.netimgerror")
			},
			cropErrorTextI18n() {
				return t("fui-image-cropper.croperror")
			},
			selectTextI18n() {
				return t("fui-image-cropper.select")
			}
		},
		data() {
			let isApp = 0;
		...
		methods: {
			...
			async compressImage(url, width, height, orientation) {
				...
				if (this.network) {
					...
					if (!imgUrl) {
						this.toast(this.netImgErrorI18n);
						return;
					}
				}
				let defaultAngle = {
			...
			rotate() {
				if (!this.src) {
					this.toast(this.selectTextI18n)
					return;
				}
				this.ani = true;
				...
				let getCutImg = async () => {
					...
					plus.zip.compressImage({
						...
						e => {
							console.log(e);
							uni.hideLoading();
							this.toast(this.cropErrorTextI18n);
						}
					);
				};
				setTimeout(() => {
