<template>
	<!--本文件由FirstUI授权予马*健（会员ID： 3 1 56，身份证尾号：01   04 3X）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
		...
		<view class="fui-calendar__btn-wrap" v-if="showBtn">
			<view class="fui-calendar__btn" :class="{'fui-calendar__diaabled':btnDisabled}"
				...
				<text class="fui-calendar__btn"
					...
					:style="{color:btnColor}" @tap.stop="handleClick">{{btnText || okTextI18n}}</text>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		computed: {
			...
			okTextI18n() {
				return t("fui-calendar.ok")
			},
			startTextI18n() {
				return t("fui-calendar.start")
			},
			endTextI18n() {
				return t("fui-calendar.end")
			},
			outofrangeTextI18n() {
				return t("fui-calendar.outofrange")
			},
			maxTextI18n() {
				return t("fui-calendar.max")
			},
			maxdaysTextI18n() {
				return t("fui-calendar.maxdays")
			}
		},
		watch: {
			width(val) {
		...
		methods: {
			...
			setLang(val) {
				const locale = this.okTextI18n == 'OK' ? 'en' : val;
				this.header = lunar.lang[`h_${locale}`] || lunar.lang.h_cn
				this.setTitle()
			},
			getWidth(val) {
				//宽度值不宜太低
			...
			swiperChange(e) {
				...
				this.month = month
				this.title = this.language === 'en' || this.okTextI18n == 'OK' ?
				...
				if (this.type == 3) {
					...
					if (values.length > 0) {
						if (values[0] == date && values[0] != values[1]) {
							let st = this.start || this.startTextI18n;
							text = st && st !== 'true' && st !== true ? st : '';

						} else if (values[1] == date) {
							let et = this.end || this.endTextI18n;
							text = et && et !== 'true' && et !== true ? et : '';
						}
					}
				}
				if (!text && !this.descrArr[idx]) {
					text = this.showLunar && this.okTextI18n != 'OK' ? this.getLunar(month, day) : ''
					if (this.isToday) {
						//今天 简繁同体
						let td = this.language === 'en' || this.okTextI18n == 'OK' ? 'Today' : '今天'
			...
			getWeekText(date) {
				...
				if (this.language === 'en' || this.okTextI18n == 'OK') {
					...
					if (type == 3) {
						...
						if (start && values.length === 1 && sts <= ets) {
							...
							if ((maxDays && maxDays > 0 && diffDays > maxDays) || maxDays === 0) {
								uni.showToast({
									title: this.maxTextI18n.replace('max', maxDays),
									icon: 'none'
								});
								return;
							}
						...
						if (idx != -1) {
							...
							if ((maxDays && maxDays > 0 && values.length >= maxDays) || maxDays === 0) {
								uni.showToast({
									title: this.maxdaysTextI18n.replace('maxdays', maxDays),
									icon: 'none'
								});
								return;
							}
