<template>
	<!--本文件由FirstUI授权予马*健（会员ID： 3 15 6，身份证尾号：  01  043X）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
...
<script>
	...
	export default {
		...
		computed: {
			...
			initI18n() {
				return t("fui-loadmore.init")
			},
			textI18n() {
				return t("fui-loadmore.text")
			},
			noneI18n() {
				return t("fui-loadmore.none")
			}
		},
		data() {
			let isNvue = false;
		...
		methods: {
			getStateText(state) {
				state = Number(state)
				const init = this.initText || this.initI18n;
				const text = this.text || this.textI18n;
				const none = this.noneText || this.noneI18n;
				return [init, text, none][state - 1]
			},
			// #ifdef APP-NVUE
			_animation() {
