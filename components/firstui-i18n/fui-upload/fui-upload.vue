	...
	<view class="fui-upload__wrap">
		<view class="fui-upload__item" :style="{width:width+'rpx',height:height+'rpx',borderRadius:radius+'rpx'}"
			...
			<view class="fui-upload__mask" v-if="status[index]!=='success' && status[index]!=='preupload' ">
				...
				<text class="fui-reupload__btn" @tap.stop="reUpload(index)"
					v-if="status[index]==='error'">{{preuploadTextI18n}}</text>
				<!-- #ifndef APP-NVUE -->
				<view class="fui-upload__loading" ref="fui_reupload_ld" v-if="status[index]==='uploading'"></view>
				<text class="fui-upload__text" v-if="status[index]==='uploading'">{{loadingTextI18n}}</text>
				<!-- #endif -->
				<!-- #ifdef APP-NVUE -->
				<text class="fui-upload__text fui-upload__mtop"
					v-if="status[index]==='uploading'">{{uploadingTextI18n}}</text>
				<!-- #endif -->
			</view>
...
<script>
	...
	export default {
		...
		computed: {
			...
			urlTextI18n() {
				return t("fui-upload.url")
			},
			deltipTextI18n() {
				return t("fui-upload.deltip")
			},
			deleteTextI18n() {
				return t("fui-upload.delete")
			},
			confirmTextI18n() {
				return t("fui-upload.confirm")
			},
			loadingTextI18n() {
				return t("fui-upload.loading")
			},
			preuploadTextI18n() {
				return t("fui-upload.preupload")
			},
			uploadingTextI18n() {
				return t("fui-upload.uploading")
			},
			maxTextI18n() {
				return t("fui-upload.max")
			},
			suffixTextI18n() {
				return t("fui-upload.suffix")
			},
			sizeTextI18n() {
				return t("fui-upload.size")
			}
		},
		methods: {
			initData(urls) {
				...
				uni.chooseImage({
					...
					success: (e) => {
						...
						for (let i = 0; i < e.tempFiles.length; i++) {
							let len = this.urls.length;
							if (len >= max && max !== -1) {
								const msg = this.maxTextI18n.replace('_max_', max)
								this.toast(msg);
								break;
							}
							//过滤图片类型
							...
							if (this.suffix.length > 0) {
								...
								if (this.suffix.indexOf(format) == -1) {
									const text = this.suffixTextI18n.replace('_suffix_', this.suffix.join(','))
									this.toast(text);
									continue;
								}
							}
							...
							if (Number(this.size) * 1024 * 1024 < size) {
								const err = this.sizeTextI18n.replace('_size_', this.size)
								this.toast(err);
								continue;
							}
							imageArr.push(path)
			...
			deleteImage(index) {
				let status = this.getStatus()
				if (status === 'uploading') {
					this.toast(this.deltipTextI18n)
				} else {
					if (this.confirmDel) {
						let _this = this
						uni.showModal({
							content: this.deleteTextI18n,
							showCancel: true,
							confirmText: this.confirmTextI18n,
							success(res) {
								if (res.confirm) {
