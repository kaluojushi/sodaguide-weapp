		...
		<view class="fui-search__bar-form" :style="{height: height+'rpx'}">
			<view class="fui-search__bar-box"
				...
				<input ref="searchBarRef" class="fui-search__bar-input" :class="{'fui-sb__input-color':!color}"
					...
					:placeholder="plholder || placeholderI18n" :value="val" :focus="isFocus" :disabled="disabled"
					confirm-type="search" @blur="inputBlur" @focus="inputFocus" @input="inputChange"
					@confirm="search" />
			...
			<view class="fui-search__bar-label" :class="[isLeft?'fui-sb__label-left':'fui-sb__label-center']"
				...
				<text class="fui-search__bar-text">{{ plholder || placeholderI18n }}</text>
			</view>
		</view>
		<text
			v-if="cancel && isSearch && !val && ((cancelText && cancelText!==true && cancelText!=='true') || cancelI18n)"
			class="fui-search__bar-btn" :style="{ color: cancelColor }"
			@tap="hideInput">{{ cancelText || cancelI18n }}</text>
		<text
			v-if="val && !disabled && isSearch && ((searchText && searchText!==true && searchText!=='true') || searchI18n)"
			class="fui-search__bar-btn" :class="{'fui-sb__btn-color':!searchColor}" :style="{ color: getSearchColor }"
			@tap="search">{{ searchText || searchI18n }}</text>
	</view>
</template>
...
<script>
	...
	export default {
		...
		computed: {
			...
			placeholderI18n() {
				return t("fui-search-bar.placeholder")
			},
			cancelI18n() {
				return t("fui-search-bar.cancel")
			},
			searchI18n() {
				return t("fui-search-bar.search")
			}
