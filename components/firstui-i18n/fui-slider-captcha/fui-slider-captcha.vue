<template>
	<!--本文件由FirstUI授权予马*健（会员ID：   3156，身份证尾号：01  04  3X）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
	<view class="fui-slider__captcha-wrap" :class="{'fui-sc__wrap-show':isShow}"
		...
		<view class="fui-slider__captcha" :style="{background:background}" @tap.stop="stop">
			<text class="fui-sc__title"
				:style="{ width: w + 'px',color: color, fontSize: size + 'rpx' }">{{ title || titleTextI18n }}</text>
			<text class="fui-sc__descr"
				:style="{width: w + 'px',color:descrColor, fontSize: descrSize + 'rpx'}">{{tipTextI18n}}</text>
...
<script>
	...
	export default {
		...
		computed: {
			...
			tipTextI18n() {
				return t("fui-slider-captcha.tip")
			},
			titleTextI18n() {
				return t("fui-slider-captcha.title")
			},
			resourceTextI18n() {
				return t("fui-slider-captcha.resource")
			},
			supportTextI18n() {
				return t("fui-slider-captcha.support")
			}
		},
		data() {
		...
		methods: {
			...
			getClipImg(imgUrl) {
				...
				plus.zip.compressImage({
					...
					e => {
						this.toast(this.resourceTextI18n);
					}
				);
			},
			// #endif
			...
			handleImage() {
				...
				if (this.imageType == 1) {
					...
					// #ifndef APP-PLUS || MP-WEIXIN || H5
					this.toast(this.supportTextI18n)
					// #endif
