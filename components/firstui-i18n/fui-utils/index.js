// 本文件由FirstUI授权予马*健（会员ID： 31  56，身份证尾号：   0104 3X）专用，请尊重知识产权，勿私下传播，违者追究法律责任。
/*!
 * 工具类：常用数据处理工具
...
const utils = {
	...
	formatTimeAgo(date, type = 1, isMs = true, suffix = '前', endUnit = 6, seconds = 10, fixedDay = true) {
		...
		if (diff > 0) {
			...
			const isZh = locale.indexOf('zh') != -1;
			...
			if (!isEmpty) {
				if (year) {
					...
					res = second < seconds ? justLang : `${second}${secondLang}${suffixLang}`
				}
			}
		}
		return res
